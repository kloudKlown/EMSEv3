...
\begin{table}[t]
	...
	\begin{tabular}{|l|lll|}
		...
		\multirow{1}{*}{Modified log variable density} & 2.0 & 1.7 & 4.4\tabularnewline
...
\begin{table}[t]
	...
	\begin{tabular}{|>{\centering}p{2.2cm}|>{\centering}p{1.3cm}|>{\centering}p{1.3cm}|>{\centering}p{1.3cm}|}
		...
		\hline 
		Relocation log density  & 73.1 & 70.7 & 47.4\tabularnewline
		\hline 
		Modified log text density & 10.5 & 13.4 & 16.8\tabularnewline
		\hline 
		Modified log variable density & 9.9  & 10.1 & 18.9\tabularnewline
		\hline 
		Modified log level density& 6.5 & 5.8  & 16.8\tabularnewline
		\hline 
	\end{tabular}
\end{table}
...
\end{enumerate}
Table~\ref{tab:dist} shows the distribution of each type in our studied projects. When there is co-occurrence of text and variable modifications, we add the change for both text and variable modification category. But, when there is co-occurrence of all three types of changes, it is not possible to determine without manual inspection if it is new log being added or modification of existing log. Hence, we categorize such logs as new logs being added. 
% When there is co-occurrence of different types of log modifications in a single log, we consider that as new log because it is difficult to categorize all the different types of co-occurrences. \ian{but your new log was defined by levenstain, so did you treat both as new log or what? this is very hard to convince people. I mean, technically, a log modification can be in different types at the same time.}
